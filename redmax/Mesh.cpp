...
dtype SDFMesh::query_SDF(Vector3 x, bool outside_accurate) {
...
Vector3 SDFMesh::query_dSDF(Vector3 x) {
...
Vector3 SDFMesh::normal(Vector3 x) {
...
Vector3 BVHMesh::normal(Vector3 x) {

    VectorX d;
    VectorXi closest_face;
    MatrixXr closest_point;
    MatrixXr closest_face_normal;

    _BVH->lookup_signed(x.transpose(), d, closest_face, closest_point, closest_face_normal);
    return closest_face_normal.row(0);
}
